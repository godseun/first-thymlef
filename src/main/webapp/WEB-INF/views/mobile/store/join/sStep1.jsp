...
<%@ include file="/WEB-INF/views/common/layout/include/base.jsp" %>
	...
	<script type="text/javascript">
		...
		$(function(){
	    	...
	    	$("#cmpNm").on("change", function() {
				ckNextStep();
	    	});
	    	$("#busiNo1").on("keyup", function() {
	    		var value  = $(this).val();
	    		var regExp = /[^0-9]/gi;
		...
		function checkBusiNo(){
			ckBusiNo = false;
 			...
 			if(busiNo.length == 12){ 
				$("#busiNoState").css("color","#3f51b5");	
				$("#busiNoState").text("사업자 번호 중복확인해 주세요.");
 				$("#chBusiNo").addClass("btnOn")
 			}else{
				$("#busiNoState").css("color","#fe0000");	
				$("#busiNoState").text("사업자 번호를 입력해 주세요.");
				$("#chBusiNo").removeClass("btnOn");
 			}
			ckNextStep();
		}
		var dupClick1 = false;
		function fnBusiNo(){
			if($("#busiNo1").val().length < 3){ 
				$("#busiNoState").css("color","#fe0000");	
				$("#busiNoState").text("사업자 번호를 입력해 주세요.");
                $("#busiNo1").focus();
				return false;
			}
			if($("#busiNo2").val().length < 2){ 
				$("#busiNoState").css("color","#fe0000");	
				$("#busiNoState").text("사업자 번호를 입력해 주세요.");
                $("#busiNo2").focus();
				return false;
			}	
			if($("#busiNo3").val().length < 5){ 
				$("#busiNoState").css("color","#fe0000");	
				$("#busiNoState").text("사업자 번호를 입력해 주세요.");
                $("#busiNo3").focus();
				return false;
			...
			var busiNo = _BT.getJoinData("", new Array("busiNo1","busiNo2","busiNo3"));
			var chkbusiNoResult = joinValidate.checkBusino(busiNo);
			if(!chkbusiNoResult){
				$("#busiNoState").css("color","#fe0000");	
				$("#busiNoState").text("사업자 번호가 유효하지 않습니다.");
				return false;
			}
			if( dupClick1 ) {
				alert("사업자 번호 중복확인 중 입니다. 잠시만 기다려주세요.");
				return false;
			}
			ckBusiNo 	= false;
			dupClick1 	= true;
			var _busiNo = _BT.getJoinData("-", new Array("busiNo1","busiNo2","busiNo3"));
			_BT.ajaxDataSubmit("post", _BT.STORE.URL.Join.chkBusiNo, {
				busiNo : _busiNo
			}, function(data) {
				dupClick1 = false; 
				if($(data).size() > 0) {
					if(_BT.isEquals(data.code, "success")) {
						if(_BT.isEquals(data.value, "CODE0")) {
							ckBusiNo = true;
							$("#busiNoState").css("color","#3f51b5");	
			    			$("#busiNoState").text("사용할수 있는 사업자 번호 입니다."); 
			    			
						}else if(_BT.isEquals(data.value, "CODE1")) { 	
			    			$("#busiNoState").text("이미 사용중인 사업자 번호 입니다.");
							$("#busiNoState").css("color","#fe0000");	
					
						}else if(_BT.isEquals(data.value, "CODE2")) { 
							$("#busiNoState").css("color","#fe0000");	
			    			$("#busiNoState").text("사용할수 없는 사업자 번호 입니다.");
						}
					}
				}
			},
			function(xhr, status, error) {
    			...
    			$("#ckIdState").text("사업자 번호 중복확인 바랍니다.");
			});
			ckNextStep();
		}
		
		function ckNextStep() {
			if(ckAgree && ckBusiNo && !_BT.isEmpty($("#cmpNm").val())){
		...
		function fnNextStep() {
			if(!_BT.isChecked("click01", "이용 약관에 대한 동의를 해 주세요.")) return;
			if(!_BT.isChecked("click02", "개인정보 수집 및 이용에 대한 동의를 해 주세요.")) return false;
			if(_BT.isEmpty($("#cmpNm").val(), "상호를 입력해주세요.")) return false;
			if(!ckBusiNo){
			...
			$("#busiNo").val(_BT.getJoinData("-", new Array("busiNo1","busiNo2","busiNo3")));
			_BT.actFormSubmit("frm", "POST", _BT.STORE.URL.Join.sStep2); 
		}
	</script>
	<div class="wid_1248">
		<div class="container">
							...
							<table class="join_form">
								...
								<tbody>
									<tr>
										...
										<td colspan="3">
											<input type="text" class="join_form_input01" id="busiNo1" name="busiNo1" maxlength="3" placeholder="3자리 숫자" style="width:50px;"/><span class="join_form_txt">-</span>
											<input type="text" class="join_form_input01" id="busiNo2" name="busiNo2" maxlength="2" placeholder="2자리 숫자" style="width:50px;"/><span class="join_form_txt">-</span>
											<input type="text" class="join_form_input01" id="busiNo3" name="busiNo3" maxlength="5" placeholder="5자리 숫자" style="width:80px;"/><span class="join_form_btn" id="chBusiNo" onclick="fnBusiNo();">중복확인</span><br>
											<span class="check_ment" id="busiNoState">사업자 등록번호 10자리 숫자를 입력해주세요.</span>								
										</td>
									</tr>
									<tr>
										<th>
										...
										<td colspan="3">
											<input type="text" class="join_form_input03" id="cmpNm" name="cmpNm" maxlength="25" placeholder="상호명을 입력해 주세요." />
										</td>
									</tr>
								</tbody>
							</table>
