		...
		switch(sAction) {  
			...
			case 'rtn':       
				...
				if(sAction == 'can'){
					...
					if(chk>0){
						alert('배송준비가 시작된 상품은 취소가 불가 합니다.\n\n고객만족센터로 문의 바랍니다.');
						return false;
					}
				}else{
					$('.ordr_item:checked').each(function( i ) {
				...
				if(!confirm('선택하신 상품을 '+jobTxt+'하시겠습니까?')){
					return false;
				}
				$('#ordrFrm').append($('<input/>', {
				    type: 'hidden',
		...
		<h1 class="title_Type03">
			...
			<div class="sub_location">
				<div class="list_location01">
					...
					<span class="arrow"><img src="${staticRoot}/img/bl_01.jpg" alt=">" /></span>
					<span class="last_txt">구매내역 상세</span>
				</div>
			</div>
		</h1>
		<div class="">
			<h1 class="title_con_Type01 margintop_30">
				결제 정보
				<c:if test="${ordrMst.PAYMETHOD eq 'Card'}">
					<span class="receipt_btn" data-value="${ordrMst.TID}">
						결제 영수증
					</span>
				</c:if>
			...
			<table class="tbl_Type03">
				...
				<tbody>
					<tr>
						...
						<td>
							 상품가격<label class="price_color"><fmt:formatNumber value="${ordrMst.TOT_PRICE}" groupingUsed="true"/></label>원
							<label class="icon">
								<img src="${staticRoot}/img/icon_plus.jpg" alt="플러스">
							</label>
							배송비<label class="price_color"><fmt:formatNumber value="${ordrMst.DEVY_FEE}" groupingUsed="true"/></label>원
						</td>
					</tr>
					<tr>
						<th>
			...
			<h1 class="title_con_Type01 margintop_50">
				...
				<span onclick="doAction('chg');" class="sButton btn_span fs-13 green marginleft_5" style="float: right;">교환</span>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<input type="text" class="join_form_input01" id="zipCode" name="zipCode" readonly="readonly" value="<c:out value="${ordrMst.ZIP_CODE}"/>"/>	
										<span class="join_form_btn01" onclick="fnPostcode();">우편번호검색</span><br/>
										<input type="text" class="join_form_input03 margintop_5" id="address" name="address" readonly="readonly" placeholder="주소" value="<c:out value="${ordrMst.ADDRESS}"/>"/>	
										<input type="text" class="join_form_input03 margintop_5 inAddr" id="dtlAddress" name="dtlAddress"  readonly="readonly" placeholder="상세주소" value="<c:out value="${ordrMst.ADDRESS_DETAIL}"/>"/>	
									</td>
								</tr>
								<tr>
									<th>
									...
									<td>
										...
										<input type="text" class="join_form_input01 mobile inAddr" id="mobile1" maxlength="4" readonly="readonly" value="${fn:split(ordrMst.TEL_NO,'-')[0]}"/>
											<span class="join_form_txt">-</span>
										<input type="text" class="join_form_input01 mobile inAddr" id="mobile2" maxlength="4" readonly="readonly" value="${fn:split(ordrMst.TEL_NO,'-')[1]}"/>
											<span class="join_form_txt">-</span>
										<input type="text" class="join_form_input01 mobile inAddr" id="mobile3" maxlength="4" readonly="readonly" value="${fn:split(ordrMst.TEL_NO,'-')[2]}"/>
									</td>
								</tr>
								<tr>
									<th>
