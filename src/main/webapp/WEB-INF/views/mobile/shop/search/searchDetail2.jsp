			...
			<ul class="tab_list" style="width: 100%;">
				<li class="tab_item" style="width: 50%;">
					<a href="javascript:;" class="tab_link" onclick="showTab('tab1');" data-type="tab1" style="width: 100%;">
						<span id="tab1" class="tab tabOn" style="width: 100%;">대화5상품</span>
					</a>
				</li>
				<li class="tab_item" style="width: 50%;">
...
<c:choose>
						<c:when test="${fn:indexOf(prdt1.PRDT_IMG, '//') >=0 }">
							<img src="<c:out value="${prdt1.PRDT_IMG}"/>" alt="상품사진" style="width:auto;height:145px; max-width:145px;" onerror="this.src='${staticRoot}/img/sample03.jpg'"/> 
</c:when>
						<c:otherwise>
										<img src="${upload}<c:out value="${prdt1.PRDT_IMG}"/>" alt="상품사진" style="width:auto;height:145px; max-width:145px;" onerror="this.src='${staticRoot}/img/sample03.jpg'"/> 
	</c:otherwise>
						</c:choose>
									</a>
								</div>
				...
				<c:choose>
					<c:when test="${query.cTp eq '3'}">
						<c:forEach items="${result.resultList2}" var="prdt2" varStatus="status2">
							...
							<div class="mItem recommend_con_Type02 <c:if test="${CNT % 2 ne '0'}"> mg_10</c:if>" data-type="<c:out value="${prdt2.PRDT_CD}"/>" style="width: 47%;">
								<div class="recommend_con_img01" align="center" style="overflow: hidden;">
									<a href="#" class="img">
<c:choose>
