...
<%@ include file="/WEB-INF/views/common/layout/include/base.jsp" %>
	<div class="main_content06" style="padding-top: 0px; min-height: 800px; margin-bottom: 20px;">
			...
			<tbody>
				<tr>
					...
					<th class="on">
						...
						<h2>
							<label class="dbCnt" data-type="E_C_5"><c:out value="${dashBoard.E_C_5}"/>  건</label> 
						</h2>
					</th>
					<th>
						<h1>
		...
		<form id="frm" name="frm">
			...
			<input type="hidden" id="jobTp" name="jobTp" value="cancelMng"/>	
			<div >
							...
							<td style="text-align: left;">
								...
								<span class="return_select4">
									<select class="" id="sSus" name="sSus">
										...
										<option value="6" selected="selected">전체 관리건</option>
										<option value="71">판매취소 요청</option>
										<option value="7">판매취소</option>
									</select>
								</span>
								<input type="text" class="input_style20 marginleft_5" style="width:200px;" id="ordrId" name="ordrId" placeholder="주문번호"/>
								<input class="input_style20 marginleft_5" style="width:150px;" type="text" id="mValue" name="mValue" placeholder="통합검색"/>
		...
		<form id="canFrm" name="canFrm">
			<input id="sPaym" name="sPaym" type="hidden" />
			<h1 class="title_con_Type01 margintop_20" style="margin-right:10px; text-align: left;">
				선택상품				
				<span style="margin-left:10px;"><input id='ck_p' name='ck_p' style="width:60px;color:#b71c1c;text-align:right;border:0;font-size:16px;" readonly="readonly"> 원</span> 
	
				<label class="icon"><img src="${staticRoot}/img/icon_plus.jpg" alt="플러스" /></label>
				<span style="">환불 배송비</span>
				<span style="margin-left:10px;"><input id='ck_d' name='ck_d' class='dely num' data-type='+' style="width:60px;color:#b71c1c;text-align:right;border:0;font-size:16px;"> 원</span> 
				
				<label class="icon"><img src="${staticRoot}/img/icon_minus.jpg" alt="마이너스" /></label>
				<span style="">추가 배송비</span>
				<span style="margin-left:10px;"><input id='mi_d' name='mi_d' class='dely num' data-type='-' style="width:60px;color:#b71c1c;text-align:right;border:0;font-size:16px;"> 원</span>  
				
				<label class="icon"><img src="${staticRoot}/img/icon_equals.jpg" alt="합계" /></label>
				<span style="">최종취소(환불)</span>
				<span style="margin-left:10px;"><input id='ck_s' name='ck_s' style="width:60px;color:#b71c1c;text-align:right;border:0;font-size:16px;" readonly="readonly"> 원</span> 
				<span> 
					<span onclick="doAction('save7');"  class="sButton btn_span fs-13 pink marginleft_5" style="float: right;">판매취소</span>
			 	</span>
			</h1>
	...
	<script type="text/javascript">
		...
		function initSheet(){
			...
			initData.Cols = [
				...
				,{'Header': '주문일',	'Type': 'Date',			'MinWidth': 52,		'SaveName': 'sMordrDt',	 		'Align': 'Center',	'Edit':0,	'Save': 0	,	'Format':'YmdHm'}
				,{'Header': '접수일',	'Type': 'Date',			'MinWidth': 52,		'SaveName': 'sMreceiptDt',	 	'Align': 'Center',	'Edit':0,	'Save': 0	,	'Format':'YmdHm'}
				,{'Header': '취소일',	'Type': 'Date',			'MinWidth': 52,		'SaveName': 'sMcanDt',	 		'Align': 'Center',	'Edit':0,	'Save': 0	,	'Format':'YmdHm'}
			...
			initData.Cols = [
				...
				,{'Header': '제조사',	'Type': 'Text',			'MinWidth': 60,		'SaveName': 'sDmakrNm', 	'Align': 'Center',	'Edit':0,	'Save': 0, 	'ColMerge':0	}
				...
				,{'Header': '취소수량',	'Type': 'Int',				'MinWidth': 60,		'SaveName': 'nQy',		 	'Align': 'Right',		'Edit':1,	'Save': 1,	'Format':'Integer', 	'ColMerge':0, 	'KeyField':1	}
				,{'Header': '구매수량',	'Type': 'Int',				'MinWidth': 60,		'SaveName': 'sDqy',		 	'Align': 'Right',		'Edit':0,	'Save': 1,	'Format':'Integer', 	'ColMerge':0	}  
				...
				,{'Header': '반송구분',	'Type': 'Combo', 		'MinWidth': 70,		'SaveName': 'sCdevyTp',	'Align': 'Center', 	'Edit':0,	'Save': 1,	'ComboText': '|반송택배 요청|직접반송 예정', 'ComboCode': '|1|2', 	'ColMerge':0,		'Hidden':dev	}  
		...
		function doAction(sAction) { 
			switch(sAction) {  
				...
				case 'save7':
					...
					if(sRow == ''){alert('판매취소 하실 주문건을 선택해 주세요.');return false;}
					...
					for(var i=0; i < sRow.length; i++){
						if(
								 mySheet2.GetCellValue(sRow[i], 'sDsttus') == '0'
							||  mySheet2.GetCellValue(sRow[i], 'sDsttus') > '2'					
						){				
							mySheet2.SetCellValue(sRow[i], 'sCheckBox', 0);
							alerted = true;
						}
					}			
					if(alerted){alert('배송전 상품만 판매취소가 가능합니다.\n\n배송정보 입력후에는 반품 프로세스로 처리 하세요.');return false;}
					if('VBank' == $('#sPaym').val()){
					...
					if(!confirm('취소 or 추가 배송비 확인철저\n\n판매취소 후 복구가 불가 합니다.\n\n선택하신 상품을 판매취소 하시겠습니까?')) return false;
					if('' == SaveStr){
						alert('판매취소 하실 주문건을 선택해 주세요.'); 
						return false;
					}else if('KeyFieldError' == SaveStr){ 
						return false;
					}else{
						var rtnData = mySheet2.GetSaveData(_BT.FRONT.URL.Order.SetCanRfd, SaveStr+ "&"+$('#canFrm').serialize()); 
						//SetRefusal
		...
		function mySheet2_OnRowSearchEnd (row) { 
			if(			mySheet2.GetCellValue(row, 'sDsttus') == '0'
			  	 	|| mySheet2.GetCellValue(row, 'sDsttus') > '2'
			) {
			...
			if(mySheet2.GetCellValue(row, 'sDsttus') == '7'){
		...
		function mySheet2_OnChange(Row, Col, Value, OldValue, RaiseFlag){ 
			...
			if(sSaveName == 'sCheckBox' || sSaveName == 'nQy'){		
				if(mySheet2.GetCellValue(Row,'nQy') == '' || mySheet2.GetCellValue(Row,'nQy') == '0'){
					...
					alert('취소하실 수량이 구매수량보다 많습니다.\n\n확인 후 처리 바랍니다.');
					mySheet2.SetCellValue(Row, 'nQy', mySheet2.GetCellValue(Row,'sDqy'));
					return false;
					
				}else{
					checkSum(Row, Col, Value, OldValue, RaiseFlag);
				}
			}
		}

		function checkSum(Row, Col, Value, OldValue, RaiseFlag){ 
			var sSaveName = mySheet2.ColSaveName(0, Col); 
			var price = 0;  
			var chk = 0 
			...
			for(var i = 1; i <= cnt; i++){
				...
				if(chk == 1){
					chk++;
					var nQy = eval(mySheet2.GetCellValue(i,'nQy'));
					var nPrice = eval(mySheet2.GetCellValue(i,'sDtotPrice'));
					price += (nQy * nPrice);
				} 
				$('#ck_p').val( price );
