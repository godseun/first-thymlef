	...
	<script type="text/javascript">
		...
		function initSheet(){
			...
			initData.Cols = [
					...
					,{'Header': '상태', 				'Type': 'Status', 		'MinWidth': 40,		'SaveName': 'sStatus', 		'Align': 'Center',	'Save': 1	, 	'Hidden':isDev 	} 
					,{'Header': '일련번호',			'Type': 'Text', 			'MinWidth': 80, 	'SaveName': 'sArtCd', 		'Align': 'Center', 	'Save': 1, 	'Edit':0, 		'DefaultValue':'NEW', 	'Hidden':isDev 	} 
					,{'Header': '프로모션명',		'Type': 'Text', 			'MinWidth': 150, 	'SaveName': 'sTitle',			'Align': 'Left',		'Save': 1, 	'Edit':1, 		'KeyField':1}
					,{'Header': '시작일',			'Type': 'Date', 			'MinWidth': 80, 	'SaveName': 'sSdt', 			'Align': 'Center', 	'Save': 1, 	'Edit':1,		'Format':'Ymd'}
					,{'Header': '종료일',			'Type': 'Date', 			'MinWidth': 80, 	'SaveName': 'sEdt', 			'Align': 'Center', 	'Save': 1, 	'Edit':1,		'Format':'Ymd'}
			...
			initData.Cols = [
					...
					,{'Header': '선택',				'Type': 'CheckBox', 	'MinWidth': 40, 	'SaveName': 'sCheckBox',		'Align': 'Center',	'Edit':1	, 	'ColMerge':0	}
			...
			initData.Cols = [
					{'Header': '삭제',				'Type': 'DelCheck', 		'MinWidth': 40, 	'SaveName': 'sDcb',				'Align': 'Center',	'Save': 1,	'Edit':1	, 	'ColMerge':0	}
					...
					,{'Header': '세일판매가',		'Type': 'Int',					'MinWidth': 50,		'SaveName': 'sSalePrice', 		'Align': 'Right',		'Save': 1	,	'Edit':1,		'Format':'Integer', 		'KeyField':1}
					...
					,{'Header': '프로모션',			'Type': 'Text', 				'MinWidth': 0,	 	'SaveName': 'sArtCd',			'Align': 'Center',	'Save': 0, 	'Hidden':isDev 	}   
					,{'Header': '구분', 				'Type': 'Seq', 				'MinWidth': 0,		'SaveName': 'sSeq', 				'Align': 'Center',	'Save': 0, 	'Hidden':isDev 	} 
					,{'Header': '품목코드',			'Type': 'Text', 				'MinWidth': 60,	 	'SaveName': 'sPcd', 				'Align': 'Center',	'Save': 1, 	'Hidden':isDev 	} 
					,{'Header': '공급사',			'Type': 'Text', 				'MinWidth': 60,	 	'SaveName': 'STORE_ID', 		'Align': 'Center',	'Save': 1, 	'Hidden':isDev 	}   
					,{'Header': '카테고리',			'Type': 'Text', 				'MinWidth': 60,	 	'SaveName': 'sCcd', 				'Align': 'Center',	'Save': 1, 	'Hidden':isDev 	} 
			];	
			...
			switch(sAction) {  
				case 'search':       
					...
					if(1 == tar){
						$('#mPrmId').val('');
						mySheet3.RemoveAll(); 
						sheet.DoSearch(_BT.FRONT.URL.Search.SheeSaleList, $('#sheetFrm').serialize()); 
					}else if(2 == tar){
						if(
						...
						if(	$('#mPrmId').val() == ''){
							alert('세일 프로모션을 선택해 주세요.');
							break; 
						}
						sheet.DoSearch(_BT.FRONT.URL.Search.SheetSalePrdtList, $('#sheetFrm3').serialize()); 
					}
				break; 
				case 'add':   
					var sheet = eval('mySheet' + tar);
				...
				case 'save':   
					...
					if(1 == tar){
						...
						var rtnData = sheet.GetSaveData(_BT.FRONT.URL.Product.SetSalePromo, SaveStr); 
						...
						if(!confirm('변경 내역을 저장하시겠습니까?')) break;
						var rtnData = sheet.GetSaveData(_BT.FRONT.URL.Product.SetSalePrdt, SaveStr + "&"+$('#sheetFrm3').serialize()); 
						sheet.LoadSaveData(rtnData);
					}
				break; 
			} 
		...
		function mySheet1_OnRowSearchEnd (row) { 
			if(mySheet1.GetCellValue(row, 'sUseYN') == 'N'){
				mySheet1.InitCellProperty(row, 'sTitle', 	{Type:"Text",Edit:0}); 
				mySheet1.InitCellProperty(row, 'sSdt', 		{Type:"Text",Edit:0}); 
				mySheet1.InitCellProperty(row, 'sEdt', 		{Type:"Text",Edit:0}); 
				mySheet1.InitCellProperty(row, 'sUseYN', 	{Type:"Text",Edit:0}); 
			}
		} 
		function mySheet1_OnDblClick(Row, Col, Value, CellX, CellY, CellW, CellH) { 
			if (Row == 0) return; //헤더행일때는 폼에 반영 안함.
			var sArtCd = mySheet1.GetCellValue( Row, 'sArtCd' );
			$('#mPrmId').val(sArtCd);
			doAction('search', '3');
		} 
					...
					<tbody>
						<tr>
							...
							<td style="text-align: left;">
								...
								<input type="text" id="sValue" name="sValue" class="input_style08 marginleft_7" placeholder="검색어 입력">							
