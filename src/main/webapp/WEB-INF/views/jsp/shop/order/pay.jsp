					...
					<c:set var="dlvyFeeH" 		value="0" />
						...
						<c:otherwise>
									...
									<td>
										...
										<input type="hidden" id="dlvyFee<c:out value="${status.count}"/>" value="${prdt1.DLVY_FEE}"/>	
										<input type="hidden" id="extraFeeTp<c:out value="${status.count}"/>" value="${prdt1.EXTRA_FEE_TP}"/>	
										<input type="hidden" id="tZone1<c:out value="${status.count}"/>" value="${prdt1.T_ZONE1}"/>	
										<input type="hidden" id="tZone2<c:out value="${status.count}"/>" value="${prdt1.T_ZONE2}"/>	
										<input type="hidden" id="tZone3<c:out value="${status.count}"/>" value="${prdt1.T_ZONE3}"/>	
									</td>
								</tr>
			...
			<c:forEach items="${cartList}" var="prdt1" varStatus="status">
				...
				<c:if test="${isFirst eq '1'}">
					<c:if test="${not status.first}">
						...
						<div class="confirmMsg2 vOff" id="confirmMsg<c:out value="${storeCnt}"/>" style="left: 550px;">
...
--%>											
							...
							<tr>
								...
								<td>
									...
									<input type="text" class="join_form_input01" id="zipCode" name="zipCode" readonly="readonly" placeholder="우편번호" value="<c:out value="${addrList[0].ZIP_CODE}"/>"/>	
<%-- 	 
...
--%>											
									...
									<input type="text" class="join_form_input03 inAddr" id="dtlAddress" name="dtlAddress"  readonly="readonly" placeholder="상세주소" value="<c:out value="${addrList[0].ADDRESS_DETAIL}"/>"/>	
								</td>
							</tr>
							<tr>
								<th>
