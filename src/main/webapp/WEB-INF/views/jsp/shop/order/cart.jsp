	...
	<script type="text/javascript">
		...
		$(function(){        
		    	...
		    	$('.cart_link').on('click', function() {
					_BT.openPopup( _BT.SHOP.URL.Search.View+'?pCode='+$(this).data('value'),'PRDT_CD', '1400', '800', 'yes', 'yes');
		    	});
						...
						<c:if test="${prdt1.DLVY_MEG ne '1'}"> 
							<c:set var="dlvyFeeTxt" 		value="개별배송 상품" />	
								...
								<tr>
									...
									<td>
										<p class="cart_txt01">											
											<span class ="cart_link" data-value="<c:out value="${prdt1.PRDT_CD}"/>">
									...
									<c:choose>
										...
										<c:otherwise>
											<td class="count_box">
												<input type="text" class="cart_count_input"  id="qy<c:out value="${status.count}"/>"  value="<c:out value="${prdt1.QY}"/>"  data-value="<c:out value="${status.count}"/>"/>
			...
			<c:if test="${fn:length(cartList) ge 2 or c12 eq 'Y'}">	
				<table class="cart_list_total margintop_20 border_top border_bottom">
					...
					<tbody>
						<tr>
							<th> [안내] </th>
							<th style="line-height:200%;text-align: left;">
								<c:if test="${fn:length(cartList) ge 2}">	
									<P>여러상품을 묶음배송으로 구매할 경우 통합배송 처리로 인해 당일발송이 불가 할 수 있습니다.</P>
								</c:if>
								<c:if test="${c12 eq 'Y'}">	
									<P>전자상거래법에 의거하여 <label class="cart_price02" >제품가치가 훼손(포장의 개봉)</label>또는 제품에 전원공급이 된 후에는 <label class="cart_price02">고객변심에 의한 반품 및 교환이 불가</label> 합니다.</P>
								</c:if>
							</th>
						</tr>
					</tbody>
