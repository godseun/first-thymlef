		...
		<div class="tab_dtl" id="dtl_tab1" data-type="tab1">
			...
			<h1 class="prdc_title02" style="margin-top: 30px;">
				<c:choose>
					...
					<c:when test="${pTp eq 'ITEMSET'}">
						#대화이 준비한 <c:out value="${query.mValue}"/>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					<c:when test="${pTp eq 'ITEMSET'}"> 
						...
						<c:forEach items="${result.resultList}" var="prdt1" varStatus="status">
							<div class="mItem recommend_con_Type02 <c:if test="${CNT % 5 ne '0'}"> mg_10</c:if>" data-type="<c:out value="${prdt1.PRDT_CD}"/>" style="height:235px">
								<div class="recommend_con_img01" align="center" style="width:215px;height:215px;">
									<a href="#" class="img">
<c:choose>
						<c:when test="${fn:indexOf(prdt1.PRDT_IMG, '//') >=0 }">
<img src="<c:out value="${prdt1.PRDT_IMG}"/>" alt="상품사진" style="width:215px;height:215px;" onerror="this.src='${staticRoot}/img/sample03.jpg'"/> 
	</c:when>
						<c:otherwise>
										<img src="${upload}<c:out value="${prdt1.PRDT_IMG}"/>" alt="상품사진" style="width:215px;height:215px;" onerror="this.src='${staticRoot}/img/sample03.jpg'"/> 
	</c:otherwise>
						</c:choose>
									</a>
								</div>
					...
					<c:when test="${pTp eq 'TIMESALE'}">
						...
						<c:forEach items="${result.resultList}" var="prdt1" varStatus="status">
							...
							<div class="mItem recommend_con_Type02 <c:if test="${CNT % 5 ne '0'}"> mg_10</c:if>" data-type="<c:out value="${prdt1.PRDT_CD}"/>" style="height:265px">
								<c:if test="${prdt1.SALE_PRICE eq '0'}">
...
<c:choose>
					...
					<c:otherwise> 
						...
						<c:forEach items="${result.resultList}" var="prdt1" varStatus="status">
							...
							<div class="mItem recommend_con_Type02 <c:if test="${CNT % 5 ne '0'}"> mg_10</c:if>" data-type="<c:out value="${prdt1.PRDT_CD}"/>">
...
<c:choose>
				...
				<br/>
				<div class="mItem recommend_con_Type02 <c:if test="${CNT % 5 ne '0'}"> mg_10</c:if>" data-type="">
					<div class="recommend_con_img01" align="center" style="overflow: hidden;">
						<a href="#" class="img">
