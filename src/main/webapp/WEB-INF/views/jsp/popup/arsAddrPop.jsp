...
<%@include file="/WEB-INF/views/common/layout/include/base.jsp" %>
	...
	<script type="text/javascript">
		...
		function fnSave(){
			...
			dupClick1 = true; 
			_BT.ajaxCallbackSubmit("frm",  _BT.FRONT.URL.Customer.AddAddr, 
				function(data) {
					dupClick1 = false;
					if($(data).size() > 0) { 
						if(_BT.isEquals(data.code, "success")){
							_BT.moveUrl(_BT.FRONT.URL.Customer.ArsAddrPop+'?usrId='+$('#usrId').val());
							return false;
						}else{
							alert(data.msg);	
							return false;
		...
		function fnDelete(){
			...
			dupClick1 = true; 
			_BT.ajaxCallbackSubmit("subFrm",  _BT.FRONT.URL.Customer.DelAddr, 
			function(data) {
				dupClick1 = false;
				if($(data).size() > 0) { 
					if(_BT.isEquals(data.code, "success")){
		...
		function fnChoice(){
			...
			$("#seq").val($("#addrId"+idx).val());
			$("#mobileNo").val($("#mobileNo"+idx).val());
			$("#receiverNo").val($("#mobileNo"+idx).val());
			$("#addrNm").val($("#addrNm"+idx).val());
			$("#receiver").val($("#receiver"+idx).val());
		...
		<div class="pop_set_content">
			<div class="pop_set_substance">
				...
				<form id="frm" name="frm">
					<input type="hidden" id="usrId" name="usrId" value="<c:out value="${pageParm.usrId}"/>"/>
