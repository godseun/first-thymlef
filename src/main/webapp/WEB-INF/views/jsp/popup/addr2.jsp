				...
				function(data) {
					...
					if($(data).size() > 0) { 
						if(_BT.isEquals(data.code, 'success')){
							_BT.moveUrl(_BT.SHOP.URL.User.popAddr2);
							return false;
						}else{
							alert(data.msg);	
							return false;
	...
	<!-- 팝업창 시작 -->
		...
		<div class="pop_set_content">
			<div class="pop_set_substance">
				...
				<table class="tbl_Type06 margintop_30">
					...
					<tbody>
						...
						<c:forEach items="${addrList}" var="addr1" varStatus="status"> 
							...
							<input type="hidden" id="sigunguCode${status.count}" value="<c:out value="${addr1.ZIP_CODE}"/>"/>	
							...
							<input type="hidden" id="mobile1${status.count}" value="<c:out value="${addr1.TEL_NO_1}"/>"/>	
							<input type="hidden" id="mobile2${status.count}" value="<c:out value="${addr1.TEL_NO_1}"/>"/>	
							<input type="hidden" id="mobile3${status.count}" value="<c:out value="${addr1.TEL_NO_1}"/>"/>	
							<tr>
								<td rowspan="2">
				...
				<div class="btn_group margintop_10">
					<span class="form_btn_Type_blue" onclick="fnChoice();">닫 기</span>
